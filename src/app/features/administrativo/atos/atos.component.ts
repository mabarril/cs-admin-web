...
@Component({
    ...
    <div class="page-container">
      <div class="page-header">
        <div>
          <h1 class="page-title">Livro de Atos</h1>
          <p class="page-subtitle">Registro de atos administrativos do clube</p>
        </div>
        <button class="btn btn-primary" (click)="abrirModal()">+ Novo Ato</button>
      </div>
            ...
            <table class="table">
              ...
              <tbody>
                @for (item of items(); track item.id) {
                  <tr>
                    <td><span class="number-badge">{{ item.act_number }}</span></td>
                    <td>{{ item.act_date | date:'dd/MM/yyyy' }}</td>
                    <td>{{ item.act_type || '—' }}</td>
      ...
      <div class="modal-overlay" (click)="fecharModal()">
        <div class="modal-box wide" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <h2>{{ itemEditando()?.id ? 'Editar Ato' : 'Novo Ato' }}</h2>
          ...
          <form (ngSubmit)="salvar()" #f="ngForm" class="modal-body">
            <div class="form-row">
              ...
              <div class="form-group">
                <label>Data *</label>
                <input class="form-control" type="date" name="act_date" [(ngModel)]="form.act_date" required />
              </div>
            </div>
            <div class="form-row">
              <div class="form-group">
              ...
              <div class="form-group">
                <label>Título *</label>
                <input class="form-control" name="title" [(ngModel)]="form.title" required placeholder="Título do ato" />
              </div>
            </div>
            <div class="form-group">
              <label>Conteúdo *</label>
              <textarea class="form-control" name="content" [(ngModel)]="form.content" rows="8" required placeholder="Texto completo do ato..."></textarea>
            </div>
    ...
    @if (itemParaExcluir()) {
      <div class="modal-overlay">
        <div class="confirm-box">
          <h3>Excluir ato?</h3>
          <p>Deseja excluir o ato <strong>Nº {{ itemParaExcluir()!.act_number }} — {{ itemParaExcluir()!.title }}</strong>? Esta ação não pode ser desfeita.</p>
  ...
  `,
    ...
    .number-badge { display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; background: #8b5cf6; color: #fff; border-radius: 50%; font-size: 0.8rem; font-weight: 700; }
